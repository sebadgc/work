...
/**
 * Página de Logs: historial de todos los logs (persistidos) por cámara,
 * con filtros por cámara, por tipo y búsqueda de texto.
 */
export default function LogsPage() {
  const { logsByCamera, cameras, clearAllLogs } = useAppContext();
  ...
  const totalCount = useMemo(
    () => Object.values(logsByCamera).reduce((n, l) => n + (l?.length || 0), 0),
    [logsByCamera],
  );
  ...
  }, [logsByCamera, cameraFilter, typeFilter, query]);
  ...
  return (
    <div className="logs-page">
      <div className="logs-page__toolbar">
        ...
        <span className="logs-page__count">{rows.length} / {totalCount}</span>
        <div className="logs-page__spacer" />
        {totalCount > 0 && (
          <Button variant="ghost" size="sm" onClick={clearAllLogs}>Limpiar todo</Button>
        )}
      </div>
        ...
        {rows.length === 0 ? (
          ...
          rows.map((r, i) => (
            <div key={`${r.camera_id}-${r.ts}-${i}`} className={`logs-row logs-row--${r.type}`}>
              <span className="logs-row__time">{r.time}</span>
